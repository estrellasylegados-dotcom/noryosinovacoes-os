...
import type { NoRemoverEtiqueta } from "@/lib/fluxo-tipos";
...
export function NoRemoverEtiquetaCard({ data }: NodeProps<Node<NoCanvasData>>) {
  const no = data.no as NoRemoverEtiqueta;
  const nome = no.etiquetaId ? (data.etiquetaNomePorId?.get(no.etiquetaId) ?? no.etiquetaId) : null;
  return (
    <>
      <Handle type="target" position={Position.Left} />
      <NoCardBase titulo="Remover etiqueta" icone={<IconeEtiqueta />} problema={data.problema} emExecucao={data.emExecucao}>
