...
/** Criar/rodar campanha de WhatsApp em massa mexe no número da clínica — só admin. */
      ...
      <div className="mx-auto max-w-4xl">
        <header className="mb-6 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-neutral-500">Campanhas de WhatsApp em massa — reativação, follow-up, avisos</p>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-neutral-100">
                {campanhas.map((c) => {
                  ...
                  const processados = c.totalEnviados + c.totalFalhas + c.totalPulados;
                  return (
                    <tr key={c.id} className="hover:bg-neutral-50">
                      <td className="px-4 py-3">
                        <Link href={`/disparos/${c.id}`} className="font-medium text-teal-700 hover:underline">
                          {c.nome}
                        </Link>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-neutral-600">
                        {processados}/{c.totalDestinatarios}
                        {c.totalFalhas > 0 && <span className="ml-1.5 text-red-600">({c.totalFalhas} falha(s))</span>}
                      </td>
