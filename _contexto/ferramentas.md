...
| Supabase (CRM OdontoMinas) | banco do CRM da OdontoMinas (projeto `odontominas-crm`, separado do Diagnóstico Digital) | Sem MCP nem CLI funcionando (mesmas limitações de sempre: MCP local `supabase-crm` some entre sessões, HTTP/OAuth pede sessão interativa, CLI logado noutra conta). **Dado (ler/escrever linha) eu alcanço direto**: REST da própria API do Supabase, autenticado com `SUPABASE_SERVICE_ROLE_KEY` de `crm/.env.local` (curl/Node local, sem precisar de você) — validado criando etiqueta e agente de IA de verdade nesta sessão. **Schema (migração, `CREATE TABLE`/`ALTER TABLE`) continua manual**: SQL Editor (`https://supabase.com/dashboard/project/exaluyarsphucotprwpi/sql/new`), você roda o SQL que eu preparo | ligada (dado, via REST); schema só manual | 2026-09-15 |
