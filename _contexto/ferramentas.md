...
| Supabase (CRM OdontoMinas) | banco do CRM da OdontoMinas (projeto `odontominas-crm`, separado do Diagnóstico Digital) | **MCP oficial (`supabase-crm-odontominas`, HTTP/OAuth, `.mcp.json` da raiz) cobre schema também, confirmado numa sessão nova**: `list_tables`/`list_migrations` leem certo, e `apply_migration` rodou a migração v12 (ALTER TABLE + CREATE TABLE) direto em produção, validado por leitura depois — não precisa mais do SQL Editor manual pra migração aditiva. Dado (ler/escrever linha) eu também alcanço direto de qualquer sessão via REST, autenticado com `SUPABASE_SERVICE_ROLE_KEY` de `crm/.env.local` — validado repetidas vezes. Login do painel por API (`/api/login`) funciona pra mintar sessão de teste; o script local de assinar cookie direto (mesmo `SESSAO_SECRET`) segue divergindo do Railway, não usar | ligada (MCP cobre dado e schema; REST direto também disponível) | 2026-09-16 |
