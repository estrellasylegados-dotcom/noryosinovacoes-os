...
- Localmente implementado e validado por gates: **Distribuição Automática V1** de conversas novas sem responsável, com round-robin, toggle por clínica, RBAC de configuração e integração no webhook Evolution. Migration `v38` aplicada e validada em produção; ainda não publicada no Railway nem validada visualmente/testada com conversa real.
...
- **Distribuição Automática V1:** implementada localmente com migration aditiva `v38`, API `/api/clinica/distribuicao`, tela `/configuracoes/distribuicao`, serviço de round-robin e integração no webhook Evolution. Migration aplicada/validada no Supabase; faltam publicar no Railway, validar a tela logado e fazer o teste real controlado.
...
3. Distribuição Automática: publicar no Railway, validar a tela `/configuracoes/distribuicao` logado e fazer teste real controlado de conversa nova; a migration `v38` já está aplicada no Supabase.
...
## Próximo passo
Escolher a próxima liberação: finalizar Automações por Kanban (responsividade, publicar, ativar flag e repetir E2E controlado), publicar Noryos Ops (deploy e validação visual logada) ou publicar Distribuição Automática (deploy e validação real controlada). Não iniciar envio real adicional sem autorização.
